...
function mountScrollWorld(container, config) {
  ...
  // [BEEF patch] Апстрим считает, что трек начинается на самом верху документа
  // (s.start = off * vh) и сравнивает это с абсолютным window.scrollY. У нас глава
  // стоит в середине страницы, поэтому все отсчёты сдвигаем на позицию контейнера.
  let base = 0;
  ...
  function layout() {
    ...
    // Высоту трека обнуляем перед замером, иначе он сам себя смещает вниз.
    track.style.height = '0px';
    base = track.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
    let off = 0;
    SEGMENTS.forEach(s => { s.start = base + off * vh; off += s.w; s.end = base + off * vh; });
    totalW = off;
  ...
  function read() {
    ...
    if (near !== activeIndex) {
      ...
      // [SMASH patch] let the host react to a scene change (we fire SFX off this).
      if (config.onSection) { try { config.onSection(near, prev); } catch (e) {} }
    }
    // [BEEF patch] прогресс и подсказка тоже считаются от начала главы, не документа
    const rel = y - base;
    scrollbarFill.style.transform = `scaleX(${clamp(rel / (totalW * vh))})`;
    hint.style.opacity = clamp(1 - rel / (0.5 * vh));
    if (particles) particles.style.transform = `translate3d(0, ${-rel * 0.05}px, 0)`;
    ticking = false;
  }
...
// [SMASH patch] ESM export so Vite can import it directly.
